...
**홈 화면 구성 (2026-01-23 최종 업데이트)**:

**✅ 표시되는 요소 (4가지만)**:
1. "지금 갈 맛집, 빠르게 결정하세요" (`.hero-compact`)
2. 신뢰 탭 (`.trust-tabs`) - 전체, 미쉐린, 유명인, 흑백요리사
3. 맛집 카드 그리드 (`.card-grid`)
4. 인라인 상세 정보 (`.inline-detail`) - 카드 클릭 시

**❌ 제거된 요소** (index.html에서 완전히 삭제됨):
- "전체 리스트 보기" 버튼 (`.home-cta`) → 사용자가 직접 네비게이션 사용
- "내 주변 맛집" 제목 (`.time-filter-section h3`)
- 시간 필터 "10분 내", "15분 내", "30분 내" (`.time-filter`) → 리스트 화면 고급 필터로 이동
- 지도 섹션 (`.map-section`)
- 트러스트 루트 서비스 소개 (`.trust-route-service`)

**중요**: 주석 처리가 아닌 **완전 삭제**되었습니다. 홈 화면은 4가지 요소만 포함합니다.

**금지 요소 (FAQ, 제보/제휴, 마이페이지에서)**:
